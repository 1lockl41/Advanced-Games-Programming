...
void RenderFrame(void)
{
	...
	/*world = XMMatrixRotationY(XMConvertToRadians(xRotation));
	world *= XMMatrixRotationX(XMConvertToRadians(xRotation));
	...
	//cb0_values.WVPMatrix = world * view * projection;
	...
	model1->Draw(&view, &projection, g_directional_light_colour, g_ambient_light_colour, g_directional_light_shines_from);
	model1->LookAt_XZ(camera1->GetX(), camera1->GetZ());
	////////////////////////////////////////////////

	/////////////RENDER SECOND CUBE///////////////////
	//world = XMMatrixRotationX(XMConvertToRadians(xRotation));
	//world *= XMMatrixTranslation(0, 1, zPosition);
	//projection = XMMatrixPerspectiveFovLH(XMConvertToRadians(45.0), 640.0 / 480.0, 1.0, 100.0);
	...
	////USE THIS TO MAKE SECOND CUBE WIREFRAME
	//////Select which primitive type to use //03 - 01
